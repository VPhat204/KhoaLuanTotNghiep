...
export default function MyCourse() {
  ...
  const fetchCourses = useCallback(async () => {
    ...
    try {
      ...
      for (const c of res.data) {
        const videosRes = await axios.get(`https://khoaluantotnghiep-i5m4.onrender.com/videos/${c.id}`);
  ...
  const fetchVideos = async (courseId) => {
    try {
      const res = await axios.get(`https://khoaluantotnghiep-i5m4.onrender.com/videos/${courseId}`);
  ...
  const handleAddVideo = async (values) => {
    try {
      if (editingVideo) {
        await axios.put(
          `https://khoaluantotnghiep-i5m4.onrender.com/videos/${editingVideo.id}`,
          { ...values },
          { headers: { Authorization: `Bearer ${token}` } }
        );
        ...
        await axios.post(
          "https://khoaluantotnghiep-i5m4.onrender.com/videos/add",
  ...
  const handleDeleteVideo = async (videoId) => {
    try {
      await axios.delete(`https://khoaluantotnghiep-i5m4.onrender.com/videos/${videoId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
