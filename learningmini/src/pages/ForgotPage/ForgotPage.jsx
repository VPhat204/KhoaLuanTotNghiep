...
function ForgotPage() {
  ...
  const handleSendOtp = async () => {
    try {
      ...
      await axios.post("http://localhost:5000/auth/forgot-password", { email: emailValue });
      setEmail(emailValue);
      setOtpSent(true);
  ...
  const handleVerifyOtp = async () => {
    try {
      ...
      await axios.post("http://localhost:5000/verify-otp", { email, otp: otpValue });
      setOtpVerified(true);
  ...
  const handleResetPassword = async () => {
    try {
      ...
      await axios.post("http://localhost:5000/reset-password", { email, password });
