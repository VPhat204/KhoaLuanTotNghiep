...
export default function CourseDetail({ course }) {
  ...
  const fetchComments = useCallback(async () => {
    if (!course?.id) return;
    try {
      // Sử dụng endpoint tree để lấy cấu trúc cây
      const res = await axios.get(
        `http://localhost:5000/comments/${course.id}/tree`,
  ...
  const handleSubmitReply = async (parentComment) => {
    if (!replyContent.trim()) return message.warning(t('translation.commentRequired'));
    ...
    try {
      ...
      message.success(t('translation.replySuccess'));
      setReplyContent("");
      setReplyingToId(null);
      fetchComments();
    } catch {
      message.error(t('translation.replyError'));
    }
  };
  ...
  const handleUpdateComment = async () => {
    if (!editContent.trim()) return message.warning(t('translation.editRequired'));
    
    try {
      await axios.put(
        `http://localhost:5000/comments/${editingComment.id}`,
        { content: editContent },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      
      message.success(t('translation.editSuccess'));
      setEditingComment(null);
      setEditContent("");
      fetchComments();
    } catch {
      message.error(t('translation.editError'));
    }
  };
  ...
  const handleDeleteComment = async () => {
    ...
    try {
      await axios.delete(
        `http://localhost:5000/comments/${commentToDelete.id}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      
      message.success(t('translation.deleteSuccess'));
      setIsDeleteModalOpen(false);
      setCommentToDelete(null);
      fetchComments();
    } catch {
      message.error(t('translation.deleteError'));
    }
  };
  ...
  const tabItems = [
    {
      key: "1",
      label: t('translation.courseDetail.tabs.overview'),
      children: (
        <>
          <Title level={5} className="course-detail-section-title">{t('courseDetail.courseDescription')}</Title>
          <Paragraph className="course-detail-description">{course.description || t('translation.courseDetail.noDescription')}</Paragraph>
        </>
      ),
    },
    {
            ...
            <List
              ...
              renderItem={(video) => (
                <List.Item>
                  ...
                  >
                    {video.duration || t('translation.courseDetail.notAvailable')}
                  </Tag>
                </List.Item>
              )}
            />
          ) : (
            <Paragraph className="course-detail-empty-state">{t('translation.courseDetail.noVideos')}</Paragraph>
          )}
        </div>
      ),
    },
    {
      key: "3",
      label: t('translation.courseDetail.tabs.comments'),
      children: (
        <div className="course-detail-comments-section">
          ...
          <div className="course-detail-comments-tree">
            {commentsTree.length > 0 ? (
              ...
              <Paragraph className="course-detail-empty-state">{t('translation.courseDetail.noComments')}</Paragraph>
            )}
          </div>
          ...
          {replyTo && !replyTo.deleted_at && (
            <div className="reply-indicator">
              <span>
                {t('translation.replyingTo')}: <strong>{replyTo.user_name}</strong>
              </span>
              <Button 
                type="text" 
                size="small" 
          ...
          <div className="course-detail-comment-actions">
            ...
            >
              {replyTo ? t('translation.sendReply') : t('courseDetail.send')}
            </Button>
            {replyTo && !replyTo.deleted_at && (
  ...
  return (
    <div className="course-detail-container">
      ...
      <Modal
        title={t('translation.confirmDeleteComment')}
      ...
      >
        <p>{t('translation.confirmDeleteCommentMessage')}</p>
      </Modal>
    </div>
  );
}